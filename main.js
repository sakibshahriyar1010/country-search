...
searchBtn.addEventListener('click', function () {
  const search = searchInput.value;
  if (search === '') {
    error.innerText = `your result cannot find`
    return
  }
  countryContainer.innerHTML = ''
...
function showData(countryArray) {
  ...
  countryArray.forEach(element => {
    ...
    div.innerHTML = `
 <div class="rounded overflow-hidden border p-2">
 <img src="${element.flag}" class="w-100 ">
 </div>
 <div class="
 py-2
 d-flex
...
">
<h1>${element.name}</h1>
<button onclick="showDetails('${element.alpha3Code}')" class="btn btn-dark">LearnMore</button>
 </div>
